#!/usr/bin/env node
// Inquiry-triage eval runner. Runs the app's real triageInquiry() on every
...
//
// Built from the claude-api skill's runner scaffold; the harness below the
// "fill these in" section is the scaffold's, plus the --ids filter.
//
...
async function loadCases() {
  const only = parseArgs(process.argv.slice(2)).ids;
  const cases = caseFile.cases.map((c) => ({ ...c, prompt: c.message }));
  return only ? cases.filter((c) => only.includes(c.id)) : cases;
}

/** Run the app's real entry point on one inquiry. */
async function runCase(input, ctx) {
  let run;
  try {
    run = await triageInquiry({
    ...
    if (/declined/i.test(e?.message ?? '')) e.failure_class = 'refusal';
    else if (/incomplete/i.test(e?.message ?? '')) e.failure_class = 'truncated';
    throw e;
  }
  return {
    output: run.result,
    transcript: [
      ...
      { role: 'assistant', content: JSON.stringify(run.result, null, 2) },
    ],
    model: run.model,
    usage: { input_tokens: run.inputTokens, output_tokens: run.outputTokens },
    stop_reason: run.stopReason,
  };
}

/** Grade one output: code checks for the fields, a Sonnet 5 judge for the reply. */
async function gradeCase(input, run) {
  const graded = await gradeTriage(input, run.output, caseFile.photographer);
  run.reply_words = graded.reply_words;
  return graded;
}

/** Side-channel perf fields beyond the built-ins (latency_s etc.). */
function perfFrom(run) { return { reply_words: run.reply_words }; }
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (k === '--ids') a.ids = val(++i).split(',').map((s) => s.trim()).filter(Boolean);
...
function usage() {
  console.error('usage: node run-eval.mjs --flow DIR --variant ID [--model ID] [--reps N] [--concurrency N] [--timeout-s N (0 = no ceiling)] [--ids a,b,c] [--approve-harness]');
}
